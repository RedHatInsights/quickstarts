...
func LogStartup(serviceName string, port string) {
...
func addPrincipal(ctx context.Context, fields logrus.Fields) {
	...
	// platform-go-middlewares v1 stores XRHID as value type with identity.Key
	if id, ok := ctx.Value(identity.Key).(identity.XRHID); ok {
