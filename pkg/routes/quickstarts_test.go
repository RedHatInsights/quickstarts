...
func TestFuzzySearch(t *testing.T) {
	...
	// Add a fuzzy search route to the router
	adapter := NewServerAdapter()
	router.Get("/fuzzy-search", func(w http.ResponseWriter, r *http.Request) {
		params := generated.GetQuickstartsFuzzySearchParams{}

		// Parse query parameters
		query := r.URL.Query()
		searchTerm := query.Get("q")
		params.Q = generated.SearchTerm(searchTerm)

		if maxDist := query.Get("max_distance"); maxDist != "" {
			if md, err := strconv.Atoi(maxDist); err == nil {
				params.MaxDistance = &md
			}
		}
		if limit := query.Get("limit"); limit != "" {
			if l, err := strconv.Atoi(limit); err == nil {
		...
		adapter.GetQuickstartsFuzzySearch(w, r, params)
	})
